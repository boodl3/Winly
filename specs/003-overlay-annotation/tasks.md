...
- [ ] T051 [US2] Wire **Escape** as the dismissal gesture: **non-consuming**, and acted on only while at least one annotation is live (research.md R3, FR-025). A consumed Escape would silently break every dialog on the machine for the annotation's lifetime. Calls the unconditional `ClearAnnotations()` — the user means whatever is in front of them, not a particular set. **The mechanism is a second `LowLevelKeyboardHookActivationMonitor` started with `KeyCombination.Parse("Escape")`, composed in `App.xaml.cs`** — that class is already generic over combinations and `VirtualKeyNames` already resolves `Escape` to `0x1B`, so this is composition and no new code in `Winly.Platform`. Do **not** add an arbitrary-key event to `IActivationKeyMonitor`: it raises `KeyDown`/`KeyUp` for the one combination it was started with, and widening it means editing the activation path that every request depends on for the sake of a dismissal gesture
